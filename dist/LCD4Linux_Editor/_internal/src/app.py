...
class LCD4LinuxEditor(tk.Tk):
    ...
    def __init__(self):
        ...
        self.selected_canvas_widget = None
    ...
    def _create_display_config(self, notebook):
        ...
        self.vnc_frame = ttk.LabelFrame(frame, text="VNC设置", padding=5)
        ...
        self.x11_frame = ttk.LabelFrame(frame, text="X11设置", padding=5)
    ...
    def browse_font(self):
        filename = filedialog.askopenfilename(title="选择字体文件", filetypes=[("TTF字体", "*.ttf *.TTC"), ("所有文件", "*.*")])
        if filename:
    ...
    def _create_widgets_panel(self, notebook):
        ...
        ttk.Button(btn_frame, text="▲", width=3, command=self.move_widget_up).pack(pady=2)
        ttk.Button(btn_frame, text="▼", width=3, command=self.move_widget_down).pack(pady=2)
        ttk.Button(btn_frame, text="✕", width=3, command=self.delete_selected_widget).pack(pady=10)
        ...
        self.widgets_tree.bind("<Button-1>", self.on_tree_select)
    ...
    def _create_right_panel(self, parent):
        ...
        direction_frame = ttk.Frame(toolbar)
        direction_frame.pack(side=tk.RIGHT, padx=5)

        ttk.Button(direction_frame, text="▲", width=2, command=lambda: self.move_widget_by_key(0, -1)).grid(row=0, column=1, padx=1, pady=0)
        ttk.Button(direction_frame, text="◀", width=2, command=lambda: self.move_widget_by_key(-1, 0)).grid(row=1, column=0, padx=1, pady=1)
        
        self.move_step_var = tk.StringVar(value="16")
        self.move_step_entry = ttk.Entry(direction_frame, textvariable=self.move_step_var, width=4, justify='center')
        self.move_step_entry.grid(row=1, column=1, padx=1, pady=1)
        
        ttk.Button(direction_frame, text="▶", width=2, command=lambda: self.move_widget_by_key(1, 0)).grid(row=1, column=2, padx=1, pady=1)
        ttk.Button(direction_frame, text="▼", width=2, command=lambda: self.move_widget_by_key(0, 1)).grid(row=2, column=1, padx=1, pady=0)
    ...
    def _draw_canvas(self):
        ...
        if self.show_grid.get():
            font_size = int(self.display_font_size.get()) if self.display_font_size.get() else self.project.display.font_size
            grid_size = max(8, int(font_size * self.canvas_scale))
            for x in range(0, scaled_width + 1, grid_size):
                self.canvas.create_line(10 + x, 10, 10 + x, 10 + scaled_height, fill="#333333", tags="grid")
                real_x = int(x / self.canvas_scale)
                if real_x % (font_size * 2) == 0:
                    self.canvas.create_text(10 + x, 10 + scaled_height + 8, text=str(real_x),
                                            fill="#666666", font=("Arial", 7), tags="grid_label")
            for y in range(0, scaled_height + 1, grid_size):
                self.canvas.create_line(10, 10 + y, 10 + scaled_width, 10 + y, fill="#333333", tags="grid")
                real_y = int(y / self.canvas_scale)
                if real_y % (font_size * 2) == 0:
                    self.canvas.create_text(10 + scaled_width + 8, 10 + y, text=str(real_y),
                                            fill="#666666", font=("Arial", 7), anchor=tk.W, tags="grid_label")
    ...
    def _draw_widgets_on_canvas(self):
        ...
        self.canvas.tag_raise("grid_label")
    ...
    def _draw_widget(self, placement, widget):
        ...
        if placement.widget_type == WidgetType.IMAGE and widget:
            image_file = getattr(widget, 'file', '')
            if image_file and os.path.exists(image_file):
                try:
                    img = Image.open(image_file)
                    orig_width, orig_height = img.size
                    width = orig_width * self.canvas_scale
                    height = orig_height * self.canvas_scale
                    img = img.resize((int(width), int(height)), Image.LANCZOS)
                    photo = ImageTk.PhotoImage(img)
                    img_tag = self.canvas.create_image(x, y, image=photo, anchor=tk.NW, tags=f"widget_{placement.widget_name}")
                    if not hasattr(self, '_images'):
                        self._images = {}
                    self._images[placement.widget_name] = photo
        ...
        if placement.widget_type == WidgetType.TEXT:
            text_content = getattr(widget, 'expression', '') or getattr(widget, 'name', 'Text')
            self.canvas.create_text(x + width/2, y + height/2, text=text_content[:10], fill="#ffffff", 
                                    font=("Arial", int(12 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")
        elif placement.widget_type == WidgetType.BAR:
            expr = getattr(widget, 'expression', '')
            bar_width = width * 0.7
            self.canvas.create_rectangle(x + 5, y + height/2 - 5, x + 5 + bar_width, y + height/2 + 5, 
                                         fill="#00ff00", outline="", tags=f"widget_{placement.widget_name}")
            if expr:
                self.canvas.create_text(x + width/2, y + height - 8, text=expr[:8], fill="#ffffff",
                                        font=("Arial", int(8 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")
        elif placement.widget_type == WidgetType.TIMER:
            expr = getattr(widget, 'expression', '') or '00:00'
            self.canvas.create_text(x + width/2, y + height/2, text=expr[:8], fill="#ffffff",
                                    font=("Arial", int(14 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")
        elif placement.widget_type == WidgetType.ICON:
            expr = getattr(widget, 'expression', '') or '★'
            self.canvas.create_text(x + width/2, y + height/2, text=expr[:8], fill="#ffffff",
                                    font=("Arial", int(16 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")
        elif placement.widget_type == WidgetType.GRAPH:
            expr = getattr(widget, 'expression', '')
            points = []
            for i in range(10):
                px = x + i * (width / 9)
                py = y + height/2 + (i % 3 - 1) * height/4
                points.extend([px, py])
            if len(points) >= 4:
                self.canvas.create_line(points, fill="#00ff00", width=2, tags=f"widget_{placement.widget_name}")
            if expr:
                self.canvas.create_text(x + width/2, y + height - 8, text=expr[:8], fill="#ffffff",
                                        font=("Arial", int(8 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")
        elif placement.widget_type == WidgetType.ARC:
            expr = getattr(widget, 'expression', '')
            self.canvas.create_arc(x + 5, y + 5, x + width - 5, y + height - 5, 
                                   start=0, extent=270, fill="#ffaa00", outline="#ffffff", 
                                   tags=f"widget_{placement.widget_name}")
            if expr:
                self.canvas.create_text(x + width/2, y + height/2 + 15, text=expr[:8], fill="#ffffff",
                                        font=("Arial", int(8 * self.canvas_scale)), tags=f"widget_{placement.widget_name}")

        bind_target = f"widget_{placement.widget_name}"
        self.canvas.tag_bind(bind_target, "<Button-1>", lambda e, w=placement.widget_name: self.select_widget_on_canvas(w))
        self.canvas.tag_bind(bind_target, "<B1-Motion>", lambda e, w=placement.widget_name: self.drag_widget(w, e))
        self.canvas.tag_bind(bind_target, "<Button-3>", lambda e, w=placement.widget_name: self.show_widget_context_menu(e, w))
    ...
    def select_widget_on_canvas(self, widget_name):
        ...
        self.selected_canvas_widget = widget_name
    ...
    def move_widget_by_key(self, dx, dy):
        selection = self.widgets_tree.selection()
        widget_name = selection[0] if selection else self.selected_canvas_widget
        if not widget_name:
            self.statusbar.config(text="请先选择要移动的组件")
            return
        placement = next((p for p in self.project.layouts[0].placements if p.widget_name == widget_name), None)
        if placement:
            try:
                step = int(self.move_step_var.get())
            except:
                step = 16
            step = max(1, step)
            placement.position.x = max(0, placement.position.x + dx * step)
            placement.position.y = max(0, placement.position.y + dy * step)
            self._draw_canvas()
            self.update_layout_info()
            self.statusbar.config(text=f"移动 {widget_name} 至 ({placement.position.x}, {placement.position.y})")
    ...
    def add_widget(self, widget_type):
        ...
        self.widgets_tree.insert("", "end", iid=name, text=name, values=(widget_type.value, "", "1000"))
        self._draw_canvas()
        self.update_layout_info()
    ...
    def edit_widget(self, event):
        ...
        if not selection:
            return
        # 获取tree item显示的名称（text属性）
        item = selection[0]
        display_name = self.widgets_tree.item(item, "text")
        # 用iid（可能是旧名称）或text来查找widget
        widget = self.widgets.get(item) or self.widgets.get(display_name)
        if widget:
            placement = next((p for p in self.project.layouts[0].placements if p.widget_name == display_name or p.widget_name == item), None)
            if placement:
                widget.x = placement.position.x
                widget.y = placement.position.y
    ...
    def _on_widget_updated(self, old_name, widget_config):
        new_name = widget_config.name
        placement = next((p for p in self.project.layouts[0].placements if p.widget_name == old_name), None)
        
        # 更新位置
        if placement:
            if hasattr(widget_config, "x"):
                placement.position.x = int(widget_config.x)
            if hasattr(widget_config, "y"):
                placement.position.y = int(widget_config.y)
            
            # 如果名称改变了，更新placement中的名称
            if old_name != new_name:
                placement.widget_name = new_name
        
        # 更新widgets字典 - 使用新名称作为key
        if old_name != new_name:
            self.widgets[new_name] = widget_config
            if old_name in self.widgets:
                del self.widgets[old_name]
        
        # 更新tree item显示（保持iid不变，只改变text和values）
        ...
        # 使用old_name作为iid来查找（因为iid没有改变）
        self.widgets_tree.item(old_name, text=new_name, 
                              values=(wtype.value, getattr(widget_config, "expression", getattr(widget_config, "file", ""))[:30], 
                                      getattr(widget_config, "update", 1000)))
        
        # 更新选中状态
        self.widgets_tree.selection_set(old_name)
        self.selected_canvas_widget = new_name
        
        self._draw_canvas()
        self.update_layout_info()
        self.statusbar.config(text=get_text("status_updated", name=new_name))
    ...
    def on_tree_select(self, event):
        selection = self.widgets_tree.selection()
        if selection:
            self.selected_canvas_widget = selection[0]
    ...
    def move_widget_up(self):
        ...
        if not selection:
            return
        widget_name = selection[0]
        items = list(self.widgets_tree.get_children())
        idx = items.index(widget_name)
        if idx > 0:
            self.widgets_tree.move(widget_name, "", idx - 1)
            placements = self.project.layouts[0].placements
            p_idx = next((i for i, p in enumerate(placements) if p.widget_name == widget_name), None)
            if p_idx is not None and p_idx > 0:
                placements[p_idx], placements[p_idx-1] = placements[p_idx-1], placements[p_idx]
            self._draw_canvas()
    ...
    def move_widget_down(self):
        ...
        if idx < len(items) - 1:
            ...
            placements = self.project.layouts[0].placements
            p_idx = next((i for i, p in enumerate(placements) if p.widget_name == widget_name), None)
            if p_idx is not None and p_idx < len(placements) - 1:
                placements[p_idx], placements[p_idx+1] = placements[p_idx+1], placements[p_idx]
            self._draw_canvas()
        ...
        try:
            ...
            for name, widget in self.widgets.items():
                ...
                try:
                    ...
                    wt = WidgetType.TEXT
                self.widgets_tree.insert("", "end", iid=name, text=name, values=(wt.value, getattr(widget, "expression", getattr(widget, "file", ""))[:30], getattr(widget, "update", 1000)))
            self.layout_combo["values"] = [l.name for l in self.project.layouts]
...
class WidgetEditorWindow(tk.Toplevel):
    ...
    def _create_general_tab(self, parent):
        ...
        ttk.Label(parent, text="X 坐标").grid(row=row, column=0, sticky=tk.W, pady=5)
        self.x_var = tk.StringVar(value=str(getattr(self.widget_config, "x", 10)))
        ttk.Entry(parent, textvariable=self.x_var, width=30).grid(row=row, column=1, sticky=tk.W, pady=5)
        row += 1

        ttk.Label(parent, text="Y 坐标").grid(row=row, column=0, sticky=tk.W, pady=5)
        self.y_var = tk.StringVar(value=str(getattr(self.widget_config, "y", 10)))
        ttk.Entry(parent, textvariable=self.y_var, width=30).grid(row=row, column=1, sticky=tk.W, pady=5)
        row += 1
        ...
        if hasattr(self.widget_config, "expression"):
            self.expr_var = tk.StringVar(value=getattr(self.widget_config, "expression", ""))
            ttk.Label(parent, text=UI_TEXT["label_expression"]).grid(row=row, column=0, sticky=tk.W, pady=5)
            expr_combo = ttk.Combobox(parent, textvariable=self.expr_var, width=28)
            expr_combo['values'] = [
                '--- CPU ---',
                'cpu', 'cpu0', 'cpu1', 'cpu2', 'cpu3',
                'cpufreq', 'cpufreq0', 'cpufreq1',
                'cpu_temp', 'cpu_temp0', 'cpu_temp1',
                '--- 内存 ---',
                'mem', 'memfree', 'memused', 'memcached', 'membuffers',
                'swap', 'swapfree', 'swapused',
                '--- 磁盘 ---',
                'disk', 'diskfree', 'diskused', 'diskpercent',
                '/', '/home', '/tmp',
                '--- 网络 ---',
                'netup', 'netdown', 'netdev',
                'eth0', 'eth1', 'wlan0', 'ppp0',
                'ip', 'hostname',
                '--- 温度 ---',
                'temp', 'temp1', 'temp2', 'temp3',
                'cpu_temp', 'sys_temp', 'gpu_temp',
                '--- 风扇 ---',
                'fan', 'fan1', 'fan2', 'fan3',
                '--- 电压 ---',
                'voltage', 'vcore', '+3.3V', '+5V', '+12V',
                '--- 电池 ---',
                'batterycap', 'batterytime', 'acpower',
                '--- 系统 ---',
                'uptime', 'load', 'load1', 'load5', 'load15',
                'proc', 'thread',
                'time', 'date', 'timestamp',
                '--- 音频 ---',
                'volume', 'mixer', 'mute',
                '--- 其他 ---',
                'brightness', 'backlight', 'contrast',
                'color', 'icon', 'progress'
            ]
            expr_combo.grid(row=row, column=1, sticky=tk.W, pady=5)
            expr_combo.bind('<<ComboboxSelected>>', self.on_expr_selected)
            row += 1
            ...
            ttk.Entry(parent, textvariable=self.precision_var, width=30).grid(row=row, column=1, sticky=tk.W, pady=5)
            row += 1
            ...
            ttk.Combobox(parent, textvariable=self.align_var, values=["L", "C", "R", "M", "A", "PL", "PC", "PR"], width=28).grid(row=row, column=1, sticky=tk.W, pady=5)
            row += 1
    ...
    def _create_graph_tab(self, parent):
        ...
        ttk.Combobox(parent, textvariable=self.graph_style_var, values=["0-线条", "1-填充"], width=28).grid(row=row, column=1, sticky=tk.W, pady=5)
        row += 1
    ...
    def on_expr_selected(self, event):
        selected = self.expr_var.get()
    ...
    def save_and_close(self):
        try:
            ...
            if hasattr(self.widget_config, "x"):
                try:
                    self.widget_config.x = int(self.x_var.get())
                except:
                    self.widget_config.x = 1
            if hasattr(self.widget_config, "y"):
                try:
                    self.widget_config.y = int(self.y_var.get())
                except:
                    self.widget_config.y = 1
            if hasattr(self.widget_config, "update"):
                try:
                    self.widget_config.update = int(self.update_var.get())
                except:
                    self.widget_config.update = 1000
            if hasattr(self.widget_config, "expression"):
                self.widget_config.expression = self.expr_var.get() if hasattr(self, 'expr_var') else getattr(self.widget_config, "expression", "")
                if hasattr(self, 'prefix_var'):
                    self.widget_config.prefix = self.prefix_var.get()
                if hasattr(self, 'postfix_var'):
